...
from .exceptions import GraphNotLoadedError, RouteNotFoundError, UnsupportedCityError
...
class SafeRouteEngine:
    ...
    def __init__(
        ...
        self._graph_dirty: bool = False  # True si la familiarité a changé depuis la dernière sérialisation
    ...
    def load_city(
        ...
        if city_key not in SUPPORTED_CITIES:
            raise UnsupportedCityError(city_key, list(SUPPORTED_CITIES.keys()))
    ...
    def record_trip(self, path: list[int]) -> None:
        ...
        if self._nx_graph is None:
            raise GraphNotLoadedError("record_trip")
        self._familiarity.update_from_path(path, self._nx_graph)
        # Marque le graphe Rust comme dirty — re-sérialisation différée au prochain compute_routes()
        # Évite de re-sérialiser 1.2M d'arcs à chaque trajet enregistré.
        self._graph_dirty = True
        logger.info("Trajet enregistré. Total trajets : %d", self._familiarity.get_familiarity_map().total_trips)
    ...
    def compute_routes(self, source_node: int, target_node: int) -> ParetoSet:
        ...
        if self._compute is None:
            raise GraphNotLoadedError("compute_routes (module Rust non compilé — lancez `maturin develop`)")

        # Re-sérialise le graphe si la familiarité a changé depuis le dernier appel
        if self._graph_dirty and self._kde_result is not None and self._PyGraph is not None:
            graph_json = self._serialize_graph(self._kde_result.risk_map)
            graph_json = self._familiarity.apply_to_graph_json(graph_json)
            self._py_graph = self._PyGraph.from_json(graph_json)
            self._graph_dirty = False
            logger.debug("Graphe Rust mis à jour avec la familiarité (lazy re-serialization)")
    ...
    def nearest_node(self, lat: float, lon: float) -> int:
        ...
        if self._nx_graph is None:
            raise GraphNotLoadedError("nearest_node")
    ...
    def get_node_coords(self, node_id: int) -> tuple[float, float]:
        ...
        if self._nx_graph is None:
            raise GraphNotLoadedError("get_node_coords")
    ...
    def get_risk_map_geojson(
        ...
        """Retourne la heatmap de risque au format GeoJSON, filtrée par bbox.

        Args:
            bbox: (min_lat, max_lat, min_lon, max_lon). Si None, retourne tout.
            max_features: Nombre maximum d'arcs retournés (défaut 5 000).
        ...
        for u, v, key, data in self._nx_graph.edges(keys=True, data=True):
            ...
            # Filtre par bounding box si fournie
            if bbox is not None:
    ...
    def _serialize_graph(self, risk_map: dict) -> str:
        ...
        for u, v, key, data in self._nx_graph.edges(keys=True, data=True):
            edges.append({
                ...
                "familiarity": 0.0,  # Sera enrichi par apply_to_graph_json()
            })
