...
def client(mock_engine):
    ...
    from saferoute.api.main import app
    import saferoute.api.routes as routes_module
...
class TestLoadCity:
    ...
    def test_load_city_invalid_raises_400(self, client, mock_engine):
        # Avec Literal["london", "cape_town"], Pydantic rejette "paris" avec 422
        # avant même d'atteindre le moteur — plus besoin de mocker load_city
        r = client.post("/api/v1/load-city", json={"city": "paris"})
        assert r.status_code == 422  # Unprocessable Entity — validation Pydantic
...
class TestRiskMap:
    ...
    def test_risk_map_returns_geojson(self, client):
        # /risk-map est maintenant POST avec RiskMapRequest (bbox + max_features)
        r = client.post("/api/v1/risk-map", json={
            "min_lat": 51.4, "max_lat": 51.6, "min_lon": -0.2, "max_lon": 0.0
        })
        assert r.status_code == 200
        data = r.json()
    ...
    def test_risk_map_no_city_raises_503(self, client_no_city):
        r = client_no_city.post("/api/v1/risk-map", json={
            "min_lat": 51.4, "max_lat": 51.6, "min_lon": -0.2, "max_lon": 0.0
        })
        assert r.status_code == 503

    def test_risk_map_max_features_respected(self, client):
        r = client.post("/api/v1/risk-map", json={
            "min_lat": 51.4, "max_lat": 51.6, "min_lon": -0.2, "max_lon": 0.0,
            "max_features": 1,
        })
...
class TestFamiliarity:
    ...
    def test_record_trip_success(self, client):
        ...
        # La réponse expose uniquement total_trips (pas les stats détaillées)
...
class TestRateLimit:
    ...
    def test_rate_limit_blocks_excess(self):
        ...
        import time
        from saferoute.api.main import _rate_store, RATE_LIMIT
